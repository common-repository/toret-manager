 ...
 */
class Toret_Manager_Sync_Process extends WP_Async_Request
{

    /**
     * Use the WP_Background_Process class to handle the sync process.
     */
    use Toret_Manager_Sync_Task;
     ...
     * @param mixed $item
     */
    function task($item): bool
    {
        $response = $this->uploadItem($item);
        $this->log($response);

        if ($this->is_processing()) {
            update_option('trman_init_sync_status', 'is_processing');
            update_option('trman_init_sync_run_clicked', 0);
        } elseif ($this->is_paused())
            update_option('trman_init_sync_status', 'paused');
        elseif ($this->is_queued())
            update_option('trman_init_sync_status', 'queued');
        elseif ($this->is_cancelled())
            update_option('trman_init_sync_status', 'cancelled');
        elseif ($this->is_queue_empty())
            update_option('trman_init_sync_status', 'queue_empty');

        $completed_tasks = get_option('trman_sync_process_completed', 0);
        $completed_tasks = $completed_tasks + 1;
        update_option('trman_sync_process_completed', $completed_tasks);

        if (get_option('trman_init_sync_status_cancel', 0) == 1) {
            $this->cancel();
        }
     ...
     * @access protected
     */
    protected $action = 'trman_initial_sync';
    ...
    protected function cancelled()
    {
        ...
        update_option('trman_init_sync_status_cancel', 0);
        update_option('trman_sync_process_total', 0);
        update_option('trman_sync_process_completed', 0);
    ...
    {
        update_option('trman_init_sync_status', 'resumed');
    ...
    public function schedule_cron_healthcheck($schedules)
    {
        ...
        if (1 === $interval) {
            $display = __('Every Minute','toret-manager');
        } else {
            /* translators: %s: minute interval */
            $display = sprintf(__('Every %d Minutes','toret-manager'), $interval);
        }
    ...
    function maybe_wp_die($return = null)
    {
        /**
